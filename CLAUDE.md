...
- Upstream tags releases `X.Y.Z_alpha` (no `v`); master usually sits one untagged patch ahead of the newest tag. `custom` tracks the **latest release tag** (currently `0.0.30_alpha`).
...
Tracking upstream tag `0.0.30_alpha` (rebased 2026-09-13); 46 commits on `custom`. Current build = `0.0.30_alpha+001` (code `300001`).
