 ...
 *
 * Copied from 自由作業盤 (`core/automation/AutomationCallers.kt`) — it is deliberately
 * self-contained and app-independent, so the forty-two sister apps answer the same question
 * the same way.
 *
 * ## Why not a token
 *
 * The token this replaces was a secret 白い熊 pasted from one app's settings into another's.
 * It cannot survive a wipe, which is fatal for the case the whole family now exists to serve:
 ...
 * `shiroikuma.evil` and pass a prefix test. Since the caller supplies the file descriptor an
 * export is written into, a prefix check would hand such an app the complete data of every sister
 * app in turn: strictly weaker than the token it replaces (応用管理, 2026-09-04, catching this in
 * review).
 *
...
object AutomationCallers {
     ...
     *
     * `signingInfo` rather than the deprecated `signatures`: a rotated key reports its whole
     * history and we want the certificate actually in force. An app with more than one current
     * signer is refused by returning null — our apps have exactly one, and "several signers, one
     * of which matches" is not a question this needs to answer.
     *
     * The pre-P branch is dead on this app (minSdk 29) but is kept as the reference has it, so a
     * sister app can copy this file back out unchanged.
     */
